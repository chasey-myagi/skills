...
两道审批门以用户对**具体根因、方案和范围**的批准为准。已在本任务、前序 skill 或可核查的交接记录中获得同一内容的批准，就引用该记录继续，不要求重复表态；普通的「修好它」不等于跳过尚未展示的根因/方案。目标或方案发生实质变化时记录前后差异，补齐受影响的批准。委派和续接必须传递原目标、当前目标、批准来源、限制及下一检查点。
...
- 什么都没有时，从 README / Makefile / package.json scripts 找测试与运行入口；结合任务上下文仍找不到必要信息时，再询问具体缺口，不猜测环境或权限。
...
先从 issue、附件、关联测试与已讨论内容提取「预期 X，实际 Y」。仍缺会影响定位的信息时询问，期间继续不依赖该答案的只读取证，不脑补着开修。
...
**向用户展示 root_cause.md，确认根因后再进入方案；已有对同一根因的明确批准时引用批准继续。**
...
**向用户展示 plan.md + 测试清单，确认后再编码；已有对同一方案及范围的明确批准时引用批准继续。**
...
先完成可审阅的 PR 描述、diff 和验证记录，再按已有授权与项目要求处理发布。需要首次具体发布确认时保留该确认；不能发布时明确记录尚未发布、阻塞原因和接续动作，不把本地实现称作已发布或已验收。用户没有授权发送的 issue 评论或其他外部消息，不由本 skill 自动授权。

## 失败模式与安全边界

- **issue 信息不足以定位**：先完成已有信息支持的取证，再列具体缺口（输入样本、版本、步骤）；依赖该信息的修复等待答案。
- **无法复现**：停在阶段 3 如实报告，给出已尝试的路径与判断（环境差异 / 疑似已修 / 需要更多信息）。
- **审批门不许静默跳过**：没有对当前根因/方案的批准，默认停在对应门；用户明确说「不用确认、直接修」才可免去这些门。已核实的同范围批准可以继承；批准来源不明不能由 agent 自称「简报已批准」来替代。
